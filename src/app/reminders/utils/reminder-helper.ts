...
export class ReminderHelper {
  ...
  public static process(
    ...
    const oneTimeReminders = reminders.filter(
      (x) => x.type == ReminderType.OneTime
    );
    _data.push(
      ...oneTimeReminders.map((x) => ({ ...x, date: parseISO(x.startDate) }))
    );
    ...
    for (let reminder of recurringReminders) {
      ...
      let _currentReminderEndDate = reminder.endDate
      ...
      if (reminder.type == ReminderType.Daily) {
        dateDiff = differenceInDays(
          _currentReminderStartDate,
          _currentReminderEndDate
        );
        key = 'days';
      } else if (reminder.type == ReminderType.Weekly) {
        dateDiff = differenceInWeeks(
          _currentReminderStartDate,
          _currentReminderEndDate
        );
        key = 'weeks';
      } else if (reminder.type == ReminderType.Monthly) {
        dateDiff = differenceInMonths(
          _currentReminderStartDate,
          _currentReminderEndDate
        );
        key = 'months';
      } else if (reminder.type == ReminderType.Yearly) {
        dateDiff = differenceInYears(
          _currentReminderStartDate,
          _currentReminderEndDate
        );
        key = 'years';
  ...
  public static distinctReminders(
    reminders: ReminderModel[],
    { startDate, endDate }: { startDate: string; endDate: string }
  ): ReminderModel[] {
    const processedData = this.process(reminders, { startDate, endDate });
    ...
    let uniqueData = ids.map((id) => {
      ...
      return {
        ...entry,
        occurenceDates: entries.map((v) => v.date),
